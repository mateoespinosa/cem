...
class TrueOptimalOld(InterventionPolicy):
    def __init__(
        self,
        concept_group_map,
        cbm,
...
class TrueOptimal(InterventionPolicy):
    ...
    def __call__(
        ...
        intervention_combinations = list(itertools.combinations(
            set(concept_group_names),
            self.num_groups_intervened,
        ))
        scores = np.zeros((x.shape[0], len(intervention_combinations)), dtype=np.float64)
        ...
        for idx in range(len(intervention_combinations)):
            ...
            for group_idx in intervention_idxs:
                real_intervention_idxs.extend(
                    sorted(self.concept_group_map[group_idx])
                )
        ...
        for sample_idx in range(x.shape[0]):
            ...
            # Set the concepts of the best-scored model to be intervened
            # for this sample
            intervention_combination = intervention_combinations[best_score_idx]
